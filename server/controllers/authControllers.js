...
async function register(req, res){
    ...
    if(!username || !email || !first_name || !last_name, !password_confirm) return res.status(422).json({'message':'Invalid fields'})
...
async function login(req, res){
    ...
    if(!user) return res.status(401).json({'message':'Email or passwor is incorrect'})

    const match = await bcrypt.compare(password, user.password)

    if(!match) return res.status(401).json({'message':'Email or passwor is incorrect'})
